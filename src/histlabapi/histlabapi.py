...
def hlab_overview(collection = None, sort = None, entity_type = None, start_date = None, end_date = None, limit = 25, run = False):   
    """
    Provides an overview of the History Lab's contents by listing all the entities of a certain type that appear in the History Lab's collection of texts.
    Users can also filter this search by a date range.
    ...
    start_date: String (in the YYYY-MM-DD format)
        A string specifying the start of the date range of the documents from which the specified entity types will be displayed. Need to provide 
        an end date as well.
    end_date: String (in the YYYY-MM-DD format)
        A string specifying the end of the date range of the documents from which the specified entity types will be displayed. Need to provide
        a start date as well.
    limit: Integer
        An integer specifying the maximum number of entities this function will display. The default limit is 25.
    run: Bool
        If False, function will only return the API URL. If True, function will return the JSON object that is generated by the full API query.
        
    Returns
    -------
    Either a string specifying the URL that is needed to query the desired results from the API, or a JSON object containing the list of all the
    entities of the specified type that appear in the collection specified within the specified date range.
...
def hlab_search(text, fields = None, join_or = False, start_date = None, end_date = None, collection = None, limit = 25, run = False):
    ...
    fields: String or list
        Series of fields that the user wants to display for each document that the search function finds. Can be input as a list if the user wishes 
        to have multiple fields displayed. If no field is provided, the function defaults to displaying the doc_id, authored, and title fields.
    join_or: Bool
        If True, the search terms will be joined by an OR connector. If False, the search terms will be joined by an AND connector. To illustrate,
        if the user inputs ['A', 'B'] in the text parameter and True in the join_or parameter, this function will return all the documents that 
        contain either 'A' or 'B'.
    start_date: String (in the YYYY-MM-DD format)
        A string specifying the start of the date range of the documents which this function will search for. Need to provide an end date as well.
    end_date: String (in the YYYY-MM-DD format)
        A string specifying the end of the date range of the documents which this function will search for. Need to provide a start date as well.
    collection: String
        A string that can be used to filter results to a specific document collection (eg. 'frus').
    limit: Integer
    ...
    -------
    Either a string specifying the URL that is needed to query the desired results from the API, or a JSON object containing the list of all the
    documents - along with the specified fields - that match the search query that the function generates.
...
def find_entity_id(entity_type, value = None):
    ...
    Returns
    -------
    A JSON object listing the names and IDs of all the entities that match the user's search term. An important thing to note is that the same entity can have
    different IDs if they appear in multiple collections. For example, Richard Nixon has three different IDs here: '190882', 'hrc5593', 'kiss190882'. Each ID
    corresponds to the version of Richard Nixon that appears in each different collection of documents. So '190882' refers to all the instances of Richard
    Nixon that appear in the FRUS documents, 'hrc5593' to the instances that appear in the Hillary Clinton emails, and 'kiss190882' to those that appear in
    the Kissinger phone calls.
...
def hlab_entity(country = None, topic = None, person = None, country_or = False, topic_or = False, person_or = False, fields = None, 
    ...
    person: String or list
        Specifies the persons that the user wants to use to search documents with. Can be input as a list if the user wishes to search for
        multiple persons. Since search terms can return multiple results (eg. there are more than one "Kennedy"s), users will need to input the
        specific person ID in this parameter. These IDs can be retrieved using the find_entity_id function.
    country_or: Bool
        If True, joins the countries users are using as their search entities with an OR connector. If False, joins them with an AND connector. To
        illustrate, if the user provides ['A', 'B'] in the parameter country and True in the country_or parameter, the function will return 
        documents that contain either the country 'A' or 'B'.
    topic_or: Bool
        If True, joins the topics users are using as their search entities with an OR connector. If False, joins them with an AND connector.
    person_or: Bool
        If True, joins the persons users are using as their search entities with an OR connector. If False, joins them with an AND connector.
    fields: String or list
        Series of fields that the user wants to display for each document that the search function finds. Can be input as a list if the user wishes 
        to have multiple fields displayed. If no field is provided, the function defaults to displaying the doc_id, authored, and title fields.
    collection: String    
    ...
    date: String (in the YYYY-MM-DD format)
        A string specfying the exact date of the documents this function will search for. Mutually exclusive with the start_date and end_date
        parameters.
    start_date: String (in the YYYY-MM-DD format)
        A string specifying the start of the date range of the documents which this function will search for. Need to provide an end date as well.
    end_date: String (in the YYYY-MM-DD format)
        A string specifying the end of the date range of the documents which this function will search for. Need to provide a start date as well.
    summary: Bool
        If True, result is a summary that count the number of documents that contain the specific entity searched for. When using summary, can only
        search for one entity at a time. Date filters also do not work.
    limit: Integer
        An integer specifying the maximum number of documents this function will display. The default limit is 25.
    run: Bool
        If False, function will only return the API URL. If True, function will return the JSON object that is generated by the full API query.
...
def hlab_id(ids, fields = None, run = False):
    ...
    ----------
    ids: String or list
        The specific ID of the documents users want to search. Can be input as a list if the user wishes to search for multipled IDs
    fields: String or list
        Series of fields that the user wants to display for each document that the search function finds. Can be input as a list if the user wishes 
        to have multiple fields displayed. If no field is provided, the function defaults to displaying the doc_id, authored, and title fields.
    run: Bool
